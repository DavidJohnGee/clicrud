...
if __name__ == '__main__':
    ...
    # enable = enable password (If you insert "", the script
    # will ask you for it)
    # password = user account password (If you insert "", the script
    # will ask you for it)
    # username = user name of account
    # host = ip address or DNS lookup
    # port = port of service (telnet = 23, ssh = 22)

    # read = buildThread(read, clicrud,
    #                    commands=["show version", "show stack"],
    #                    method='telnet', fileoutput=True, fileformat='string',
    #                    username="admin", host="192.168.10.52", port="23",
    ...
    #                   username="admin", host="192.168.10.52", port=22,
    #                   enable="Passw0rd", password="Passw0rd",
    #                   return_type="string")
    ...
    #                    username="admin", host="192.168.10.52", port=22,
    #                    enable="Passw0rd", password="Passw0rd",
    #                    return_type="string",type='icx6610')

    # Generic usage - this works for MLX, CER_CES
    cer_ces_1 = buildThread(read, clicrud, command="show version",
                            method='ssh', fileoutput=True,
                            fileformat='string', username="admin",
                            host="192.0.2.1", port=22, enable="Passw0rd",
                            password="Passw0rd", return_type="string")

    cer_ces_2 = buildThread(read, clicrud, command="show version",
                            method='ssh', fileoutput=True,
                            fileformat='string', username="admin",
                            host="192.0.2.2", port=22, enable="Passw0rd",
                            password="Passw0rd", return_type="string")

    # Start does multiple things. Adds the function to the thread list,
    # start processes and enters a loop state
    ...
    # This does not immediately stop anything. Stop if the CLI loop exits
    clicrud.stop()
